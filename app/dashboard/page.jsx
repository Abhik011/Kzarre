...
const StatCard = ({ title, value, change, icon: Icon, isPositive, bgColor }) => (
  <div className="bg-white rounded-2xl p-6 border-l-4 border-blue-400 shadow-sm">
    <div className="flex items-start justify-between">
      <div>
        <p className="text-gray-600 text-sm font-medium">{title}</p>
        <h3 className="text-3xl font-bold text-gray-900 mt-2">{value}</h3>
        <p className={`text-sm mt-2 flex items-center gap-1 ${isPositive ? 'text-teal-600' : 'text-red-600'}`}>
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        {/* Users Chart */}
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <h3 className="text-lg font-bold text-gray-900 mb-4">New vs Returning Users</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-bold text-gray-900">Deals Details</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">Piece</th>
            ...
            <tbody>
              {dealsData.map((deal) => (
                <tr key={deal.id} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 font-medium">{deal.piece}</td>
                  <td className="px-6 py-4 text-sm text-gray-900 font-medium">{deal.amount}</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-sm font-semibold ${deal.statusColor}`}>
                      {deal.status}
                    </span>
                  </td>
                </tr>
